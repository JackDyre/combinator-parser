...
fn main() -> Result<()> {
    ...
    let (rest, (_name, _args)) = declaration(input_str)?;
    ...
    let (rest, exp) = all_consuming(expression)(rest)?;
...
fn expression(s: &str) -> IResult<&str, Vec<Element>> {
    many1(element)(s)
}
...
fn declaration(s: &str) -> IResult<&str, (&str, Vec<&str>)> {
